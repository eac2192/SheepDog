...
<<<<<<< HEAD
	...
	public Point many_dogs_strategy(Point[] dogs, Point[] sheeps) {
		...
		case 1: // unused
			...
			return this.move_straight(dogs[id - 1], new Point(100, 50),
					MAX_SPEED);
		case 2: // unused
	...
	public Point chaseFurthestFromGoal(ArrayList<Point> sheeps, Point dest,
		...
		for (Point sheep : sheeps) {
			...
			if (dist_to >= furthest_dist && !(sheep.x < 50.0)
					&& (ndogs >= 3 || ((id == 1 && ndogs != 1) || tmpdis < 10))) {
	...
	public boolean isValid(Point dest) {
		if (dest.x > 100 || dest.x < 0 || dest.y > 100 || dest.y < 0
				|| hitTheFence(pos, dest)) {
			return false;
		} else {
			return true;
		}
...
=======
    ...
    public final Point BOTTOM_LEFT = new Point(0, 100);
    public final Point BOTTOM_RIGHT = new Point(100, 100);
>>>>>>> 77ce36bb1be9545f84819c786d13b9fe2ba24ef5
